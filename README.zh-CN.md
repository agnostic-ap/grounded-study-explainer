...
它坚持一条严格规则：模型只能引用宿主应用提供的稳定证据 ID。格式错误、越权引用或缺少必要不确定性说明时，核心会返回类型明确的失败结果，而不是把不合格解释展示给用户。

## 真实产品接入效果

下图来自一个使用本库的下游备考产品。宿主应用负责读取已审核内容、分配证据 ID、实施配额、记录质量日志和渲染 UI；这些产品专属层不包含在本库中。
...
核心没有 Fastify、Express、Drizzle、React、数据库或特定模型厂商的运行时依赖。
...
源码已经发布到 GitHub，目前不宣称已发布 npm registry 包。
...
不要相信浏览器提交的题干、正确答案或证据白名单。应当在服务端读取已审核记录，并由可信代码分配稳定 ID。
...
这些结果来自 fixture provider 的确定性核心契约检查，**不是**真实模型质量、线上成功率、事实准确率或网络延迟指标。

## 安全与隐私清单

- 在可信服务端检索证据并构建白名单。
- 把题目、答案和证据字符串都当作模型侧不可信数据。
- 失败时不要把被拒绝的模型原文当作降级内容展示。
- 不要把私有记录、学习者数据、完整 Prompt 或 API Key 写入日志或公开样本。
- 可以记录模型版本、延迟、格式有效性、引用违规、评测分数和失败原因等运行元数据。
- 鉴权、权限、配额、限流和滥用防护必须由宿主应用负责。

## 非目标

本库不负责数据检索、用户认证、配额、具体模型调用、日志持久化或 UI，也不能证明模型内容在事实层面绝对正确。它不要求向量数据库、多 Agent、微调或 MCP。
